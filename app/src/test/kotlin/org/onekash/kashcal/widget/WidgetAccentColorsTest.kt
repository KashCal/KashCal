...
/**
 * Verifies the widget color resolution: seed source yields providers, dynamic yields null (so the
 * widget keeps Material You), and a legacy teal user is migrated onto the seed path.
 */
class WidgetAccentColorsTest {
    ...
    @Test
    fun `dynamic source yields null so the widget keeps Material You`() = runTest {
        val providers = resolveWidgetAccentColors(
    ...
    @Test
    fun `unset source defaults to null (dynamic)`() = runTest {
